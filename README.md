...
Codey 的核心设计是把大模型能力与真实业务场景做结合，通过简单的工具和技能编排即可完成对 AI 能力的嵌入，无需搭建知识向量库等重型第三方组件，在低代码平台、表单自动填写、系统自动化等业务场景中有良好表现。
...
- QQ群：`313275628`
- 在线演示：**[http://118.89.57.141:8090](http://118.89.57.141:8090/examples)**

## 项目定位

Codey 面向需要把 AI 能力嵌入真实业务流程的系统，重点覆盖低代码平台和带项目上下文的工程场景。

- 面向低代码平台：把表单结构、字段定义、业务上下文、历史数据、校验规则等能力接入 AI，让 AI 能理解“页面背后是什么业务对象”。
- 面向工程场景：把工作目录、源码文件、会话状态、工具能力和上下文资料统一提供给 AI，提升理解、生成、修改和执行的有效性。
...
- 工具与技能编排：通过统一工具抽象和技能编排承载读写文件、搜索代码、工作区操作等能力。
- 业务语义接入：让 AI 不只理解文本输入，也理解表单、字段、业务规则和项目上下文。
...
- 系统自动化与业务流程中的 AI 辅助执行
- 带项目上下文的代码理解、代码修改与任务执行
...
当前仓库包含可直接演示的前后端样例，用于展示 Codey 在业务场景和项目场景中的接入方式。

### 1. 业务示例

业务示例展示 AI 如何基于表单上下文和业务语义完成辅助处理。
...
自定义表单示例体现低代码平台中表单结构与 AI 能力的结合方式。
...
很多 AI 产品默认把输入理解为单纯的对话或纯源码工程，但真实业务系统往往同时包含业务语义、项目上下文和可执行工具。

- 低代码平台里，AI 需要理解表单、字段、业务实体、校验规则和用户操作上下文。
- 工程场景里，AI 需要理解工作目录、配置文件、模块边界和已有代码结构。
- 单纯接入模型并不能解决问题，关键是让 AI 同时拿到上下文、业务语义和工具能力。
...
- 持续完善工程场景下的工作区协作与代码修改能力
- 持续增强会话恢复、事件追踪、验证与工具治理能力
- 持续沉淀可复用的 Skill 与项目上下文接入方式
